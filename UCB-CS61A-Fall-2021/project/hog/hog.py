...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while i <= num_rolls:
        ...
        if single_outcome == 1:
            is_roll_1 = True
        total, i = total + single_outcome, i + 1
    if is_roll_1: return 1
    return total
    # END PROBLEM 1
...
def gcd(a, b):
    assert a > 0 and b > 0, "The arguments in GCD must be positive."
    i = min(a, b)
    while i > 1:
        if a % i == 0 and b % i == 0:
            return i
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    # BEGIN PROBLEM 5
    def own_strategy(player):
        if player:
            roll_num = strategy1(own_score(player), opponent_score(player))
        else:
            roll_num = strategy0(own_score(player), opponent_score(player))
        return roll_num
    
    def own_score(player):
        return score1 if player else score0
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    # BEGIN PROBLEM 7
    def update(who, score):
        difference = score - last_score
        if difference > running_high:
            print(f"{difference} point(s)! The most yet for Player {who}") 
            return announce_highest(who, score, difference)
        else:
            return announce_highest(who, score, running_high)

    def say(score0, score1):
        if who: return update(who, score1)
        else: return update(who, score0)
    return say
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    """
    # BEGIN PROBLEM 9
    max_avg_score, num_rolls = 0, 1
    desired_num = num_rolls
    average_roll_dice = make_averaged(roll_dice, trials_count)
    while num_rolls <= 10:
        cur_avg_score = average_roll_dice(num_rolls, dice)
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    prob_score = score + free_bacon(opponent_score)
    if extra_turn(prob_score, opponent_score): return 0
